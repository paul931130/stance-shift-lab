...
介面分成四個分頁：**設定與資料**（資料來源、資料集、實驗表單）、**執行與紀錄**（Agent 終端、研究佇列、實驗詳情）、**統計與 Preregistration**（同協議統計、凍結樣本）、**即時觀察**（Finnhub 即時報價，與歷史回測完全分開）。目前分頁與檢視中的實驗都會反映在網址列（`?tab=`、`?selected=`），可重新整理或收藏。

...
這份流程只啟用歷史研究與回測，系統不會連接券商或送出交易指令。
...
開啟 <http://127.0.0.1:8000/>。`/health` 與頁面標籤都應顯示目前的研究協議版本（見 [版本紀錄](../CHANGELOG.md) 最上方）。

## 2. 建立新版資料集

目前版本的正式資料需要 900 個日曆日的歷史行情、可比較的 SEC 財務指標，以及完整的固定版本 FinBERT 標題分數。舊協議版本採集的資料集會保留，但通常只適合稽核或敏感性測試。

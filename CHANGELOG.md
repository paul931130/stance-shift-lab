...
## v3-0913.1（後續，2026-09-16）— 再清一輪：flow-tabs、4 個 Agent 卡片、版型衝突

不改變決策提示或可引用證據，因此不升版。

- 移除舊版子分頁按鈕列（`.flow-tabs` 的「1 資料準備／2 建立實驗」），改由點擊主導覽列直接跳轉；資料下載完成後現在會自動跳到「建立實驗」區塊（原本呼叫的 `setFlow('experiment')` 在連續捲動版面下其實什麼都不會發生，現在真的會捲過去）。
- `#collection-agents`（4 個資料 Agent 狀態卡片）與「資料流程」說明段落同樣被 CSS 焊死看不到、但邏輯照常在跑，直接刪掉；改寫 `renderCollectionAgents()` 只保留真正算出「資料集齊不齊」判斷所需的部分。
- **順手修好一個問題**：`#data-readiness`（選定資料集後顯示的即時品質提示，例如「FinBERT 50/50」「SEC 基本面缺少可比較期間」）先前也被 CSS 藏起來，這份資訊沒有在別處重複出現，現在讓它重新顯示。
- 修正兩條互相衝突的 `@media(max-width:1000px){.workspace-grid{...}}` 規則（一個設單欄、一個設「310px 側欄＋內容」的舊版兩欄版型殘留），版面本來就已經是單欄連續捲動，兩欄那條是死碼，直接移除。

驗證：全新 build 後 106/106 Python 測試、10/10 JS 測試通過；瀏覽器實測確認導覽跳轉、`#data-readiness` 顯示正常，無 console 錯誤。
